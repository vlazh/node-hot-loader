...
class HmrClient {
  ...
  static logApplyResult(updatedModules, renewedModules) {
    ...
    if (!renewedModules || renewedModules.length === 0) {
      ...
      if (numberIds) { console.log(HmrClient.logPrefix, 'Consider using the NamedModulesPlugin for module names.'); }
    }
  }
  ...
  run(listener = this.defaultListener) {
    if (!module.hot) { throw new Error(HmrClient.logPrefix, 'Hot Module Replacement is disabled.'); }
